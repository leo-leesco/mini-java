...
type constant = Cbool of bool | Cint of int32 | Cstring of string
type pexpr_typ = PTboolean | PTint | PTident of ident
...
type pstmt = { pstmt_desc : pstmt_desc; pstmt_loc : location }
...
type typ = Tvoid | Tnull | Tboolean | Tint | Tclass of class_
...
(** All the occurrences of the same attribute point to a single record of the
    following type. *)
...
(** All the occurrences of the same variable point to a single record of the
    following type. *)
...
type decl = Dconstructor of var list * stmt | Dmethod of method_ * stmt
